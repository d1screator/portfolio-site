...
const COL = "site";
const loadDoc = async (k: string, fb: any) => { try { const s = await getDoc(doc(db, COL, k)); return s.exists() ? s.data().value : fb; } catch { return fb; } };
const saveDoc = async (k: string, v: any) => { try { await setDoc(doc(db, COL, k), { value: v }); } catch (e) { console.error(e); } };
const subDoc = (k: string, cb: (v: any) => void) => onSnapshot(doc(db, COL, k), s => { if (s.exists()) cb(s.data().value); });
const uploadFile = async (file: File, folder = "uploads") => {
...
const LIGHT_THEME = {
  ...
  text: "#0a0a0f",
  textSecondary: "rgba(10,10,15,0.5)",
  textMuted: "rgba(10,10,15,0.25)",
  accent: "#0a0a0f",
  accentGlow: "rgba(0,0,0,0.06)",
  navBg: "rgba(248,248,250,0.88)",
  cardBg: "rgba(0,0,0,0.035)",
  inputBg: "rgba(0,0,0,0.05)",
  scrollbar: "rgba(0,0,0,0.2)",
  orb1: "rgba(0,0,0,0.04)",
  orb2: "rgba(0,0,0,0.025)",
  decorText: "rgba(0,0,0,0.04)",
  reviewDate: "rgba(10,10,15,0.35)",
};

type Theme = typeof DARK_THEME;

// ─── Global Styles ───────────────────────────────────────────────────────────
const makeGlobalStyles = (t: Theme, isDark: boolean) => `
...
@keyframes themeToggle{from{transform:scale(.8) rotate(-20deg);opacity:0}to{transform:scale(1) rotate(0deg);opacity:1}}
...
.anim-theme{animation:themeToggle .3s cubic-bezier(.16,1,.3,1) both}
.glass{
...
.card-hover:hover{
  ...
  box-shadow:${isDark ? "0 24px 64px rgba(255,255,255,0.06)" : "0 24px 64px rgba(0,0,0,0.12)"};
...
#scroll-progress{
  ...
  box-shadow:${isDark ? "0 0 12px rgba(255,255,255,0.6)" : "0 0 12px rgba(0,0,0,0.4)"};
}
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
const ScrollProgress = ({ t }: { t: Theme }) => {
...
const AnimCounter = ({ target, suffix = "", duration = 1400 }: { target: number; suffix?: string; duration?: number }) => {
...
const Stars = ({ rating, onRate, size = 18 }: { rating: number; onRate?: (v: number) => void; size?: number }) => (
  <div style={{ display: "flex", gap: 3 }}>
    {[1, 2, 3, 4, 5].map(i => (
      <span key={i} onClick={() => onRate?.(i)}
        ...
        onMouseEnter={e => onRate && ((e.target as HTMLElement).style.transform = "scale(1.3)")}
        onMouseLeave={e => onRate && ((e.target as HTMLElement).style.transform = "scale(1)")}>★</span>
    ))}
  </div>
);

// ─── Theme Toggle ────────────────────────────────────────────────────────────
const ThemeToggle = ({ isDark, onToggle, t }: { isDark: boolean; onToggle: () => void; t: Theme }) => (
  <button
    onClick={onToggle}
    style={{
      ...
      fontSize: 22,
      boxShadow: isDark ? "0 8px 32px rgba(255,255,255,0.08)" : "0 8px 32px rgba(0,0,0,0.15)",
  ...
  >
    <span className="anim-theme" key={String(isDark)}>{isDark ? "☀️" : "🌙"}</span>
  </button>
);

// ─── Section Title ───────────────────────────────────────────────────────────
const SectionTitle = ({ children, id, sub, t }: { children: React.ReactNode; id?: string; sub?: string; t: Theme }) => (
...
// ─── Button ──────────────────────────────────────────────────────────────────
const Btn = ({ children, onClick, variant = "primary", style: s = {}, disabled = false, t }: {
  children: React.ReactNode; onClick?: () => void; variant?: string; style?: React.CSSProperties; disabled?: boolean; t: Theme;
}) => {
  ...
  return (
    <button onClick={disabled ? undefined : onClick} style={{ ...base, ...vars[variant], ...s }}
      onMouseEnter={e => { if (disabled) return; e.currentTarget.style.transform = "translateY(-3px) scale(1.02)"; e.currentTarget.style.boxShadow = isDark ? "0 12px 40px rgba(255,255,255,0.15)" : "0 12px 40px rgba(0,0,0,0.2)"; }}
      onMouseLeave={e => { if (disabled) return; e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = (vars[variant] as any).boxShadow || ""; }}
    >{children}</button>
  );
};

// ─── Marquee ─────────────────────────────────────────────────────────────────
const Marquee = ({ items, t }: { items: string[]; t: Theme }) => {
...
// ─── File Upload ─────────────────────────────────────────────────────────────
const FileUploadBtn = ({ onUpload, accept = "video/*,image/*", label = "Загрузить", t }: { onUpload: (url: string) => void; accept?: string; label?: string; t: Theme }) => {
...
const makeInputStyle = (t: Theme): React.CSSProperties => ({
...
const makeLabelStyle = (t: Theme): React.CSSProperties => ({
  display: "block", fontSize: 10, fontWeight: 700, color: t.textMuted,
  textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 6,
});
const iconBtnStyle: React.CSSProperties = { background: "transparent", border: "none", cursor: "pointer", fontSize: 16, padding: 6, fontFamily: "'Manrope',sans-serif" };

// ─── Decorative grid lines ────────────────────────────────────────────────────
const GridLines = ({ t }: { t: Theme }) => (
...
// ─── Cross decorators ─────────────────────────────────────────────────────────
const Cross = ({ t, style: s = {} }: { t: Theme; style?: React.CSSProperties }) => (
...
export default function App() {
  ...
  const globalStyles = makeGlobalStyles(t, isDark);
  ...
  const [customWorks, setCustomWorks] = useState<string | null>(null);
  const [customChannels, setCustomChannels] = useState<string | null>(null);
  ...
  const saveReviews = async (r: typeof reviews) => { setReviews(r); await saveDoc("reviews", r); };
  const saveWorks = async (w: typeof works) => { setWorks(w); await saveDoc("works", w); };
  const saveChannels = async (c: typeof channels) => { setChannels(c); await saveDoc("channels", c); };
  const saveContacts = async (c: typeof contacts) => { setContacts(c); await saveDoc("contacts", c); };
      ...
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 48, flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <Btn variant="secondary" onClick={() => setIsAdmin(false)} t={t}>← На сайт</Btn>
          </div>
        </div>
        ...
        {adminTab === "works" && <div className="anim-fade-up">
          {(["reels", "motion", "youtube"] as const).map(cat => (
              ...
              {works[cat].map((item, idx) => (
                <div key={item.id} style={{ background: t.surface, borderRadius: 14, padding: 18, marginBottom: 10, border: `1px solid ${t.border}` }}>
                  ...
                  <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                    <FileUploadBtn accept="video/*" label="Видео" onUpload={url => { const nw = { ...works }; nw[cat] = [...nw[cat]]; nw[cat][idx] = { ...nw[cat][idx], video: url }; saveWorks(nw); }} t={t} />
            ...
            <div key={ch.id} style={{ background: t.surface, borderRadius: 16, padding: 24, marginBottom: 20, border: `1px solid ${t.border}` }}>
              ...
              {ch.points.map((pt, ptIdx) => (
                <div key={ptIdx} style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center", flexWrap: "wrap" }}>
                  ...
                  <FileUploadBtn accept="image/*" label="Превью" onUpload={url => { const nc = [...channels]; const pts = [...nc[chIdx].points]; pts[ptIdx] = { ...pts[ptIdx], thumb: url }; nc[chIdx] = { ...nc[chIdx], points: pts }; saveChannels(nc); }} t={t} />
        ...
        {adminTab === "settings" && <div className="anim-fade-up" style={{ background: t.surface, borderRadius: 16, padding: 28, border: `1px solid ${t.border}` }}>
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <FileUploadBtn accept="image/*" label="Загрузить логотип" onUpload={url => { setLogoUrl(url); saveDoc("logo", url); }} t={t} />
  ...
  return (
    <div style={{ background: t.bg, minHeight: "100vh", position: "relative", overflow: "hidden", transition: "background .4s ease" }}>
      <style>{globalStyles}</style>
      <ScrollProgress t={t} />
      <ThemeToggle isDark={isDark} onToggle={() => setIsDark(d => !d)} t={t} />
      ...
      <nav style={{
        ...
        <Btn onClick={() => scrollTo("contacts-section")} style={{ padding: "9px 22px", fontSize: 11 }} t={t}>Заказать</Btn>
      </nav>
      ...
      <section style={{
        ...
        {/* Cross decorators */}
        <Cross t={t} style={{ top: "18%", left: "12%" }} />
        <Cross t={t} style={{ top: "18%", right: "12%" }} />
        <Cross t={t} style={{ bottom: "22%", left: "8%" }} />
        <Cross t={t} style={{ bottom: "22%", right: "8%" }} />
        ...
        <div className="anim-scale-in" style={{ position: "relative", zIndex: 2, marginBottom: 32 }}>
          <div style={{
            ...
            boxShadow: isDark ? "0 0 60px rgba(255,255,255,0.06), 0 0 120px rgba(255,255,255,0.03)" : "0 0 60px rgba(0,0,0,0.08)",
        ...
        <div className="anim-fade-up" style={{ animationDelay: ".35s", display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center", position: "relative", zIndex: 2 }}>
          <Btn onClick={() => scrollTo("portfolio-section")} t={t}>Смотреть работы</Btn>
          <Btn variant="secondary" onClick={() => scrollTo("contacts-section")} t={t}>Написать нам</Btn>
        </div>
      ...
      <section id="portfolio-section" style={{ padding: "112px 20px", maxWidth: 1200, margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        <SectionTitle sub="Избранные проекты из нашего портфолио" t={t}>Портфолио</SectionTitle>

        {[
          { key: "reels" as const, label: "Reels & Shorts", desc: "Вертикальные форматы для соц. сетей", num: "01" },
          { key: "motion" as const, label: "Моушн-графика", desc: "Анимация и визуальные эффекты", num: "02" },
          { key: "youtube" as const, label: "YouTube", desc: "Полноформатный монтаж", num: "03" },
          ...
          <div key={key} style={{ marginBottom: 80 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: key === "youtube" ? "repeat(auto-fill,minmax(320px,1fr))" : "repeat(auto-fill,minmax(200px,1fr))", gap: 16 }}>
              {works[key].map((item, i) => (
                <div key={item.id} className="anim-fade-up card-hover glass" style={{
                  animationDelay: `${i * .08}s`, borderRadius: 16, overflow: "hidden",
                  border: `1px solid ${t.border}`,
                }}>
      ...
      {/* ── RESULTS ── */}
      <section id="channels-section" style={{ padding: "112px 20px", position: "relative", zIndex: 1, background: isDark ? "rgba(255,255,255,0.015)" : "rgba(0,0,0,0.02)", borderTop: `1px solid ${t.border}`, borderBottom: `1px solid ${t.border}` }}>
        <div style={{ maxWidth: 1120, margin: "0 auto" }}>
          ...
          <SectionTitle sub="Реальные результаты каналов после работы с нами" t={t}>Результаты</SectionTitle>

          {channels.map((ch, idx) => (
            <div key={ch.id} className="anim-fade-up glass" style={{
              animationDelay: `${idx * .12}s`, borderRadius: 20,
              padding: "36px 32px", marginBottom: 24, border: `1px solid ${t.border}`,
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 32 }}>
                <div style={{ background: isDark ? "rgba(248,113,113,0.05)" : "rgba(220,38,38,0.04)", borderRadius: 14, padding: "20px 24px", border: `1px solid ${isDark ? "rgba(248,113,113,0.1)" : "rgba(220,38,38,0.1)"}` }}>
                ...
                <div style={{ background: isDark ? "rgba(74,222,128,0.05)" : "rgba(22,163,74,0.04)", borderRadius: 14, padding: "20px 24px", border: `1px solid ${isDark ? "rgba(74,222,128,0.15)" : "rgba(22,163,74,0.15)"}` }}>
                  ...
                  <AreaChart data={ch.points} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <defs>
                      <linearGradient id={`grad-${ch.id}`} x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor={isDark ? "rgba(255,255,255,0.3)" : "rgba(0,0,0,0.2)"} stopOpacity={1} />
                        <stop offset="100%" stopColor={isDark ? "rgba(255,255,255,0)" : "rgba(0,0,0,0)"} stopOpacity={0} />
                      </linearGradient>
                    </defs>
                    <XAxis dataKey="label" tick={{ fill: t.textMuted as string, fontSize: 11, fontFamily: "'Manrope',sans-serif" }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fill: t.textMuted as string, fontSize: 10, fontFamily: "'Manrope',sans-serif" }} axisLine={false} tickLine={false} />
                    <Tooltip
                    ...
                    />
                    <Area type="monotone" dataKey="views" stroke={isDark ? "rgba(255,255,255,0.6)" : "rgba(0,0,0,0.5)"} strokeWidth={2} fill={`url(#grad-${ch.id})`} dot={{ r: 4, fill: t.text as string, stroke: t.bg as string, strokeWidth: 2 }} activeDot={{ r: 6 }} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
      ...
      <section id="reviews-section" style={{ padding: "112px 20px", maxWidth: 960, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <SectionTitle sub="Что говорят клиенты о нашей работе" t={t}>Отзывы</SectionTitle>
        ...
        <div className="glass" style={{ borderRadius: 20, padding: "32px 36px", marginBottom: 48, border: `1px solid ${t.border}`, position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", gap: 14, marginBottom: 14, flexWrap: "wrap", alignItems: "center" }}>
            <input placeholder="Ваше имя" value={reviewName} onChange={e => setReviewName(e.target.value)} style={{ ...makeInputStyle(t), flex: 1, minWidth: 160 }} />
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <textarea placeholder="Напишите отзыв..." value={reviewText} onChange={e => setReviewText(e.target.value)} rows={3} style={{ ...makeInputStyle(t), resize: "vertical", marginBottom: 18 }} />
          <Btn onClick={submitReview} disabled={saving} style={{ fontSize: 11, padding: "11px 28px" }} t={t}>{saving ? "Отправка..." : "Отправить"}</Btn>
        </div>

        {/* Review grid */}
        <div style={{ display: "grid", gap: 14 }}>
          {reviews.map((r, i) => (
            <div key={r.id} className="anim-fade-up card-hover glass" style={{
              animationDelay: `${i * .06}s`, borderRadius: 18,
              padding: "24px 28px", border: `1px solid ${t.border}`,
              position: "relative", overflow: "hidden",
            }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14, gap: 12, flexWrap: "wrap" }}>
                ...
                <span style={{ color: t.reviewDate as string, fontSize: 11, fontFamily: "'Manrope',sans-serif", flexShrink: 0 }}>{r.date}</span>
              </div>
      ...
      <section id="contacts-section" style={{ padding: "112px 20px 140px", maxWidth: 800, margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        <SectionTitle sub="Готовы обсудить ваш проект — напишите нам" t={t}>Контакты</SectionTitle>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 16, marginBottom: 48, position: "relative", zIndex: 2 }}>
          ...
          <a href={`https://t.me/${(contacts.telegram || "").replace("@", "")}`} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none" }}>
            <div className="glass card-hover" style={{
              borderRadius: 20, padding: "32px 28px", border: `1px solid ${t.border}`,
        ...
        <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap", position: "relative", zIndex: 2 }}>
          <Btn variant="tg" onClick={() => window.open(`https://t.me/${(contacts.telegram || "").replace("@", "")}`, `_blank`)} style={{ fontSize: 12, padding: "16px 36px" }} t={t}>✈️ Написать в Telegram</Btn>
          <Btn variant="secondary" onClick={() => window.location.href = `mailto:${contacts.email || "hello@studio.com"}`} style={{ fontSize: 12, padding: "16px 36px" }} t={t}>📧 Написать на Email</Btn>
        </div>
      </section>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(0,0,0,0.8)", backdropFilter: "blur(20px)", display: "flex", alignItems: "center", justifyContent: "center", animation: "fadeIn .3s ease" }}
          ...
          <div className="anim-scale-in glass-strong" onClick={e => e.stopPropagation()} style={{ borderRadius: 24, padding: "44px 40px", width: 360, border: `1px solid ${t.borderStrong}`, textAlign: "center" }}>
            ...
            <input type="password" placeholder="Пароль" value={pwdInput} onChange={e => { setPwdInput(e.target.value); setPwdError(false); }}
              ...
              style={{ ...makeInputStyle(t), textAlign: "center", fontSize: 16, marginBottom: 12, letterSpacing: 4 }} autoFocus />
            {pwdError && <p style={{ color: "#f87171", fontSize: 12, marginBottom: 14, fontFamily: "'Manrope',sans-serif" }}>Неверный пароль</p>}
            <Btn onClick={handleLogin} style={{ width: "100%" }} t={t}>Войти</Btn>
          </div>
        </div>
      )}
    </div>
